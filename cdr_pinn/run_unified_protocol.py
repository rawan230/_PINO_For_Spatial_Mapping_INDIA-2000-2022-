"""
CANONICAL CDR-PINO training / evaluation code (audit of record 2026-09-24/25).

Ported from the audit's verified runner (results/code/cdr_unified_runner.py in the
project-root repository). Every Step 8 number that is quoted in the paper comes from
this script (plus analyze_unified.py and term_magnitudes.py). train_standard_protocol.py
and run_validation_tracks.py are kept only to reproduce the historical (v1) numbers.

What the unified protocol fixes (audit findings):
  * Historical Track A (train_standard_protocol.py) and Tracks B1/B2/B3
    (run_validation_tracks.py) used DIFFERENT protocols (A: AdamW + ReduceLROnPlateau,
    80 epochs; B: Adam + cosine, 50-epoch budget for B1/B2) and were separately trained
    models. -> ONE protocol (Track A's validated one) is applied to every track.
  * Historical B3 supervised the terminal (LSE-pooled) data loss with fire_ever pooled
    over ALL 2000-2022 months, including the held-out test years (label leakage).
    -> For B3 the terminal label is rebuilt from TRAINING (fit) months only.
    "B3orig" re-runs the historical leaky label for comparison only.
  * Historical B1/B2 carved validation pixels at random from the training region, so
    early stopping monitored an in-distribution score. -> Validation for spatial tracks
    is carved as whole 2-degree blocks from the training region.
  * Ablation, B1, B2 and B3 were single-seed. -> Seeds 42/43/44; fold / region /
    held-out-year assignment is fixed (SPLIT_SEED=42), so seeds vary only model
    initialisation and training noise.
  * No per-cell predictions were saved. -> Every run saves its test-cell (or
    test-cell-month) predictions and its checkpoint, for paired tests.

The model, the losses, the covariates and the grid are NOT changed (model.py,
losses.py, spectral_ops.py, preprocessing.py are used as-is). Loss facts, as
implemented: data loss = pos-weighted BCE over all training cells (no negative
sampling) + 0.5/0.5 mix with an LSE-pooled terminal BCE in the last window; BC =
mean |grad u|^2 on the boundary ring (a du/dn proxy); IC loss is identically 0
(u_0 = 0); loss weights w_i <- 0.9 w_i + 0.1 mean_norm/norm_i every 5 windows.

Physics configurations (identical budget / protocol):
  nophys   : data + IC losses only (PDE and BC removed)
  diff     : + PDE residual with the diffusion term only (+ BC)
  diffadv  : + diffusion + advection
  full     : + diffusion + advection + reaction   (full CDR)

Outputs (default Physics_Informed_FireRisk_Model/CDR_PINN_Data/unified/):
  meta.json, partitions.npz, track_<T>.json, pred_<tag>.npz, ckpt_<tag>.pt

Usage (GPU, cdr_pinn_env):
  python run_unified_protocol.py                              # everything (~102 runs)
  python run_unified_protocol.py --tracks A --configs full --seeds 42
  python run_unified_protocol.py --device cpu --epochs 1 --tracks A --configs nophys --seeds 42 --out <tmp>
"""
import argparse
import hashlib
import json
...
from build_monthly_stacks import LON_MIN, LON_MAX, LAT_MIN, LAT_MAX, TARGET_H, TARGET_W  # noqa: E402
from preprocessing import (DATA_PATH, load_tensors, covariate_stack, physics_covariates,  # noqa: E402
                           grid_metadata, build_masks_3way)

OUT = os.path.join(STEP8_DIR, "CDR_PINN_Data", "unified")

# Track A's validated protocol, applied to every track
WINDOW, N_EPOCHS, WIDTH, N_LAYERS, MODES, LR = 24, 80, 32, 4, 16, 1e-3
WEIGHT_DECAY = 0.0          # validated winner, CDR_PINN_Data/cdr_pinn_weight_decay_search.json
VAL_EVERY, PATIENCE = 5, 4  # early stopping on validation AUC: 4 checks x 5 epochs without improvement
SPLIT_SEED = 42             # fold / region / year / pixel assignment -- fixed across seeds and configs
VAL_FRAC = 0.1875           # of the training portion -> ~65/15/20 overall
CONFIGS = {
...
def random_pixel_split(ctx):
    ...
    _, _, tr, va, te = build_masks_3way(ctx.ndvi_f1_np)
    return tr, va, te


def carve_block_validation(ctx, train_mask, frac=VAL_FRAC, seed=SPLIT_SEED):
    """Spatially blocked validation: whole 2-degree blocks from the training region."""
...
def b3_years(ctx, test_frac=0.2):
    """IDENTICAL held-out years to run_validation_tracks.run_track_b3; validation years
    are carved from the remaining training years."""
    years = ctx.years[:-1]
    uy = np.unique(years)
    ...
    n_val = max(1, int(round(len(train_years) * VAL_FRAC)))
...
def train_eval(ctx, cfg_name, seed, fit_pix, val_pix, test_pix,
    ...
    # terminal label: pooled over ALL months (spatial tracks / historical B3orig) or over
    # FIT months only (leak-free B3). Prediction index ti -> label month ti+1.
    if terminal_label_from_fit_months:
        idx = np.where(fit_months)[0] + 1
    ...
    def score(s, pix, months):
        ...
        Temporal track: per-cell-month score vs the monthly indicator over `months`."""
        m = ctx.valid & pix
        if not temporal:
    ...
    for ep in range(N_EPOCHS):
        ...
        for start in range(0, T - 1, WINDOW):
            ...
            L_ic = ic_loss(u) if start == 0 else torch.zeros((), device=dev)  # identically 0 (u_0 = 0)
...
def main():
    ...
    meta = dict(protocol_date="2026-09-25 (ported from audit runner 2026-09-24)",
                script=os.path.abspath(__file__), code_commit=git_commit(STEP8_DIR),
                data=DATA_PATH, data_sha256=sha256(DATA_PATH), torch=torch.__version__,
                ...
                protocol=dict(window=WINDOW, epochs=N_EPOCHS, width=WIDTH, layers=N_LAYERS, modes=MODES, lr=LR,
                              ...
                              scheduler="ReduceLROnPlateau(val BCE, factor .5, patience 2 checks)",
                              early_stop="val AUC, check every 5 ep, patience 4 checks", split_seed=SPLIT_SEED,
                              spatial_validation="whole 2deg blocks carved from the training region",
                              b3_terminal_label="fit months only (B3); all months (B3orig, historical)"))
    save("meta", meta)
    ...
    np.savez_compressed(os.path.join(OUT, "partitions.npz"), valid=ctx.valid, block_id=ctx.block_id,
                        ...
                        b3_fit_months=fit_m, b3_val_months=val_m, b3_test_months=test_m)
    ...
    for track in args.tracks.split(","):
        ...
        if track == "A":
            ...
            leakfree = track == "B3"
            cfgs = configs if leakfree else [c for c in ("nophys", "full") if c in configs] or ["nophys", "full"]
            for c in cfgs:
                for s in seeds:
